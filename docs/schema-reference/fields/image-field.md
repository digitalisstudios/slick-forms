...
| `field_type` | `string` | Yes | - | Must be 'image' for this field type |
...
| `input_mode` | `select` | No | `"url"` | Input Mode |
| `image_url` | `text` | No | - | Image URL |
| `image_upload` | `file` | No | - | Upload Image |
| `image_preview` | `html` | No | - | Preview |
| `alt_text` | `text` | No | - | Alternative text for accessibility |
| `width` | `text` | No | - | Width |
| `height` | `text` | No | - | Height |
| `aspect_ratio` | `select` | No | `""` | Force a specific aspect ratio |
| `object_fit` | `select` | No | `""` | How the image should fit within its container |
...
**`field_type`** *(required)*
- Must be 'image' for this field type
...
```php
{
    ...
    "label": "Profile Photo",
    "placeholder": null,
    "help_text": "Recommended size: 800x800px (max 2MB)",
    "is_required": false,
    "order": 0
}
```
...
```json
{
    ...
    "properties": {
        ...
        "field_type": {
            ...
            "storage": "column",
            "value": "image",
            "description": "Must be 'image' for this field type"
        },
        "validation_rules": {
            "type": "mixed",
            "storage": "column",
